...
class DRL_Scheduler(Scheduler):
    ...
    def __init__(self, hfs_env, stage, noise_model=None, *args, **kwargs):
    ...
    def select_job(self, machine, step=0, *args, **kwargs):

        # determine queue
        queue = machine.global_queue
        # compute priorities of all jobs in queue
        states, actions = self.compute_priorities(machine, queue, step)
        # compute rewards
    ...
    def compute_priorities(self, machine, queue, step=0, *args, **kwargs):

        # initialize states
        states = np.stack([self.create_features(job, machine) for job in queue], axis=0)
        # compute actions
        actions = self.model(states).numpy()
        # add noise if noise model is given
        if self.noise_model:
            actions = self.noise_model(actions, step)
...
class FIFO_Scheduler(Scheduler):
    ...
    def select_job(self, machine):
        ...
        # determine queue
        queue = machine.global_queue
        # determine dummy priorities
        dummy_prios=np.array([(-1) * i for i in range(len(queue))])
        ...
        if self.stage == "smd":
            ...
            job = self.reallocate_and_reselect(
                job, machine, queue, priorities=dummy_prios
            )
        # save selected job as active job of machine
        machine.job = job
...
class LIFO_Scheduler(Scheduler):
    ...
    def select_job(self, machine):
        ...
        queue = machine.global_queue
        # determine dummy priorities
        dummy_prios = np.array([i for i in range(len(queue) + 1)])
        # identify index of job
...
class EDD_Scheduler(Scheduler):
    ...
    def select_job(self, machine):
        ...
        queue = machine.global_queue
        # determine dummy priorities
        dummy_prios = np.array([(-1) * job.due_date for job in queue])
...
class MST_Scheduler(Scheduler):

    """
    Selects jobs from a queue according to the minimum slack time
    ...
    def __init__(self, hfs_env, stage, *args, **kwargs):
        ...
        super(MST_Scheduler, self).__init__(hfs_env, stage, *args, **kwargs)
    ...
    def select_job(self, machine):
        ...
        if self.stage == 'smd':
            #print(self.hfs_env.env.__dict__)
            # determine dummy priorities
            dummy_prios = np.array([(-1) * (job.due_date - self.hfs_env.env.now() - job.t_smd - job.t_aoi) for job in queue])
            job_index = queue.index(min(queue, key=lambda job: (job.due_date - self.hfs_env.env.now() - job.t_smd - job.t_aoi)))
        else:
            # determine dummy priorities
            dummy_prios = np.array([(-1) * (job.due_date - self.hfs_env.env.now() - job.t_aoi) for job in queue])
            job_index = queue.index(min(queue, key=lambda job: (job.due_date - self.hfs_env.env.now() - job.t_aoi)))    
        # take job from queue
        job = queue.pop(job_index)
        # if smd scheduler
        if self.stage == "smd":
...
class CR_Scheduler(Scheduler):

    """
    Selects jobs from a queue according to the minimum critical ratio
    dispatching strategy.
    -----------------------------------------------------------------
    Parameters:
    -----------------------------------------------------------------
    ...
    def __init__(self, hfs_env, stage, *args, **kwargs):
        ...
        super(CR_Scheduler, self).__init__(hfs_env, stage, *args, **kwargs)
...
class SPT_Scheduler(Scheduler):
    ...
    def select_job(self, machine):
        ...
        if self.stage == "smd":
            # determine dummy priorities
            dummy_prios = np.array([(-1) * job.t_smd for job in queue])
            job_index = queue.index(min(queue, key=lambda job: job.t_smd))
        else:
            # determine dummy priorities
            dummy_prios = np.array([(-1) * job.t_aoi for job in queue])
...
class SSTEDD_Scheduler(Scheduler):

    """
    Selects jobs from a queue according to the shortest setup time
    dispatching strategy. Uses EDD as a tiebreaker In case of equal setup times.
    -----------------------------------------------------------------
    Parameters:
    -----------------------------------------------------------------
    hfs_env: hybrid flow shop environment where the Scheduler shall
    ...
    def __init__(self, hfs_env, stage, *args, **kwargs):
        ...
        super(SSTEDD_Scheduler, self).__init__(hfs_env, stage, *args, **kwargs)

    def select_job(self, machine):
        # determine queue
        queue = machine.global_queue
        # determine dummy priorities
        minor = sorted([job for job in queue if job.family == machine.setuptype], key=lambda job: job.due_date)
        major = sorted([job for job in queue if job.family != machine.setuptype], key=lambda job: job.due_date)
        joblist = np.array(minor + major)
        dummy_prios=np.array([(-1) * i for i in range(len(joblist))])
        # identify index of job with lowest score
        job_index = queue.index(joblist[0])
        # take job from queue
        job = queue.pop(job_index)
        # if smd scheduler
        if self.stage == "smd":
...
class SSTSPT_Scheduler(Scheduler):

    """
    Selects jobs from a queue according to the shortest setup time
    dispatching strategy. Uses SPT as a tiebreaker In case of equal setup times.
    -----------------------------------------------------------------
    Parameters:
    -----------------------------------------------------------------
    hfs_env: hybrid flow shop environment where the Scheduler shall
    ...
    def __init__(self, hfs_env, stage, *args, **kwargs):
        ...
        super(SSTSPT_Scheduler, self).__init__(hfs_env, stage, *args, **kwargs)

    def select_job(self, machine):
        # determine queue
        queue = machine.global_queue
        # determine dummy priorities
        minor = sorted([job for job in queue if job.family == machine.setuptype], key=lambda job: job.t_smd)
        major = sorted([job for job in queue if job.family != machine.setuptype], key=lambda job: job.t_smd)
        joblist = np.array(minor + major)
        dummy_prios=np.array([(-1) * i for i in range(len(joblist))])
        # identify index of job with lowest score
        job_index = queue.index(joblist[0])
...
class SSTMST_Scheduler(Scheduler):

    """
    Selects jobs from a queue according to the shortest setup time
    dispatching strategy. Uses MST as a tiebreaker In case of equal setup times.
    -----------------------------------------------------------------
    Parameters:
    -----------------------------------------------------------------
    hfs_env: hybrid flow shop environment where the Scheduler shall
    ...
    def __init__(self, hfs_env, stage, *args, **kwargs):
        ...
        super(SSTMST_Scheduler, self).__init__(hfs_env, stage, *args, **kwargs)

    def select_job(self, machine):
        # determine queue
        queue = machine.global_queue
        # determine dummy priorities
        minor = sorted([job for job in queue if job.family == machine.setuptype], key=lambda job: (job.due_date - self.hfs_env.env.now() - job.t_smd - job.t_aoi))
        major = sorted([job for job in queue if job.family != machine.setuptype], key=lambda job: (job.due_date - self.hfs_env.env.now() - job.t_smd - job.t_aoi))
        joblist = np.array(minor + major)
        dummy_prios=np.array([(-1) * i for i in range(len(joblist))])
        # identify index of job with lowest score
        job_index = queue.index(joblist[0])
...
class SSTCR_Scheduler(Scheduler):

    """
    Selects jobs from a queue according to the shortest setup time
    dispatching strategy. Uses CR as a tiebreaker In case of equal setup times.
    -----------------------------------------------------------------
    Parameters:
    -----------------------------------------------------------------
    hfs_env: hybrid flow shop environment where the Scheduler shall
    ...
    def __init__(self, hfs_env, stage, *args, **kwargs):
        ...
        super(SSTCR_Scheduler, self).__init__(hfs_env, stage, *args, **kwargs)
...
class SMD_12Inputs_DRL_Scheduler(DRL_Scheduler):
    ...
    def create_features(self, job, machine, *args, **kwargs):
        ...
        features = np.array(job_features + same_setups + workload_features)
        # return features
        return features
