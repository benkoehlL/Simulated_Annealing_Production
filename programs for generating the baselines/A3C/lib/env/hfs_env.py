...
class Job(sim.Component):
    def setup(
        ...
        self.smd_start_time = None
        self.smd_finish_time = None

        # AOI statistics
        self.selected_aoi = None
        self.aoi_start_time = None
        self.aoi_finish_time = None
...
class SMD(sim.Component):
    ...
    def process(self):
        ...
        while True:
            ...
            if not self.global_queue and not self.local_queue:
                self.status = "idle"
                if self.hfs_env.env.animate() == True:
                    self.set_status(status=self.status)
                yield self.passivate()
            ...
            if self.job != None:
                ...
                self.job.selected_smd = self.name()
                self.job.smd_start_time = self.hfs_env.env.now()
                if self.hfs_env.env.animate() == True:
                    self.hfs_env.global_smd_queue.update_info_text()
                ...
                if self.setuptype == self.job.family:
                    self.status = "minor setup"
                    if self.hfs_env.env.animate() == True:
                        self.set_status(status=self.status)
                    yield self.hold(20)
                else:
                    self.status = "major setup"
                ...
                self.job.smd_finish_time = self.hfs_env.env.now()
                self.production_plan.append((self.job.id, self.job.smd_start_time, self.job.smd_finish_time))
                self.job.activate()
                self.job = None
    ...
    def calc_workload(self):
        ...
        elif self.status in ["minor setup", "major setup"]:
    ...
    def process(self):
        ...
        while True:
            ...
            if self.job != None:
                ...
                self.job.selected_aoi = self.name()
                self.job.aoi_start_time = self.hfs_env.env.now()
                if self.hfs_env.env.animate() == True:
                    self.hfs_env.global_aoi_queue.update_info_text()
                    self.job.draw_animation(
                        x=self.img_x + (self.img_w / 2), y=self.img_y + (self.img_h / 2)
                ...
                self.job.aoi_finish_time = self.hfs_env.env.now()
                self.production_plan.append((self.job.id, self.job.aoi_start_time, self.job.aoi_finish_time))
                self.job.activate()
                self.job = None
...
class HFS_Env:
    def __init__(
        ...
        problem,
        scaling,
        smd_scheduling_class,
        aoi_scheduling_class,
        ...
        # embed schedulers
        self.smd_scheduler = smd_scheduling_class(hfs_env=self, stage="smd")
        self.aoi_scheduler = aoi_scheduling_class(hfs_env=self, stage="aoi")
